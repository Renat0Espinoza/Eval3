...
import com.example.Eval3.Servicio.MuebleServicio;
...
public class MuebleController {
    ...
    public ResponseEntity<Mueble> activarMueble(@PathVariable Long id) {
        return new ResponseEntity<>(muebleServicio.activarMueble(id), HttpStatus.OK);
    }

    @PatchMapping("/{id}/stock")
    public ResponseEntity<Mueble> agregarStock(@PathVariable Long id, @RequestParam int cantidad) {
        return new ResponseEntity<>(muebleServicio.agregarStock(id, cantidad), HttpStatus.OK);
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Void> eliminarMueble(@PathVariable Long id) {
        muebleServicio.eliminarMueble(id);
